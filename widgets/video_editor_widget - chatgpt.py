...
class VideoEditorWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        QShortcut(QKeySequence("+"),            self, activated=lambda: self._nudge_speed(+0.10))
        QShortcut(QKeySequence("-"),            self, activated=lambda: self._nudge_speed(-0.10))
        QShortcut(QKeySequence(Qt.Key_Plus),    self, activated=lambda: self._nudge_speed(+0.10))
        QShortcut(QKeySequence(Qt.Key_Minus),   self, activated=lambda: self._nudge_speed(-0.10))
        QShortcut(QKeySequence(Qt.Key_Equal),   self, activated=lambda: self._nudge_speed(+0.10))  # Shift+='+' Layouts
        # Speed reset: 1 (setzt 1.00x)
        QShortcut(QKeySequence("1"),            self, activated=lambda: self.set_playback_rate(1.0))

        # Viewport Zoom: Ctrl + / Ctrl - / Ctrl 0
        ...
        def _add_speed_shortcut(seq, delta):
            sc = QShortcut(QKeySequence(seq), self, activated=lambda: self._nudge_speed(delta))
            sc.setContext(Qt.ApplicationShortcut)

        # schneller (+0.10x)
        _add_speed_shortcut("+",           +0.10)   # Text-Variante
        _add_speed_shortcut("=",           +0.10)   # Shift+='+' Layouts
        _add_speed_shortcut(Qt.Key_Plus,   +0.10)   # Haupttastatur
        _add_speed_shortcut(Qt.Key_KP_Add, +0.10)   # Numpad

        # langsamer (-0.10x)
        _add_speed_shortcut("-",               -0.10)
        _add_speed_shortcut(Qt.Key_Minus,      -0.10)
        _add_speed_shortcut(Qt.Key_KP_Subtract,-0.10)

        # Reset bleibt wie gehabt
        sc_reset = QShortcut(QKeySequence("1"), self, activated=lambda: self.set_playback_rate(1.0))
    ...
    def _nudge_speed(self, delta: float):
        ...
        try:
            ...
            cur = 1.0
        new = max(0.10, min(4.00, cur + delta))  # Klammern 0.10x .. 4.00x
